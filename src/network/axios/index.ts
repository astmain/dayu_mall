  ...
  private initInterceptors() {
    ...
    this.axiosInstance.interceptors.request.use(
      ...
      (error) => {
        ...
        error.code == "ERR_NETWORK" && message.error("设备离线,请检查网络")
      ...
      (response) => {
        ...
        if (data?.result) {
          if (data.result?.code === 403) {
            // 401 未授权 清空用户信息和token 跳转登录
            useUserStore.getState().resetUser()
            window.location.href = "/login"
            return Promise.reject(data.result)
          }
          if (data.result?.code === 500) {
            // message.error("接口不存在,请检查URL")
            return data.result?.message
          }
          return data.result //因为下方封装默认泛型默认定义到了response下的data下的resBaseInfo下的data
        }
